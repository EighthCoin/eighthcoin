...
void MultisigDialog::on_createAddressButton_clicked()
{
    ...
    for(int i = 0; i < ui->pubkeyEntries->count(); i++)
    {
        ...
        CEighthCoincoinAddress address(strAddressEntered);
    ...
    CEighthCoincoinAddress address(scriptID);
    ...
    for(int i = 0; i < ui->outputs->count(); i++)
    {
        ...
        if(entry)
        {
            if(entry->validate())
            {
                ...
                CEighthCoincoinAddress address(recipient.address.toStdString());
...
void MultisigDialog::on_transaction_textChanged()
{
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout)
    {
        ...
        CEighthCoincoinAddress address(addr);
...
void MultisigDialog::on_sendTransactionButton_clicked()
{
    ...
    if(fee < minFee)
    {
        QMessageBox::StandardButton ret = QMessageBox::question(this, tr("Confirm send transaction"), tr("The fee of the transaction (%1 8TH) is smaller than the expected fee (%2 DEV). Do you want to send the transaction anyway?").arg((double) fee / COIN).arg((double) minFee / COIN), QMessageBox::Yes | QMessageBox::Cancel, QMessageBox::Cancel);
        if(ret != QMessageBox::Yes)
            return;
    }
    else if(fee > minFee)
    {
        QMessageBox::StandardButton ret = QMessageBox::question(this, tr("Confirm send transaction"), tr("The fee of the transaction (%1 8TH) is bigger than the expected fee (%2 DEV). Do you want to send the transaction anyway?").arg((double) fee / COIN).arg((double) minFee / COIN), QMessageBox::Yes | QMessageBox::Cancel, QMessageBox::Cancel);
        if(ret != QMessageBox::Yes)
            return;
    }
